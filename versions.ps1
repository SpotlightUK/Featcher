...
# Generate release notes by extracting GitHub commit comments and turning them into Markdown
(git log "$latest_tag..HEAD" --format="* %s (%an / %ar)") -replace "\[#(.+)\]",'[#$1](https://www.pivotaltracker.com/story/show/$1)' -replace "_","\_" > release_notes.md
# (git log "$latest_tag..HEAD" --format="* %s (%an / %ar)") |  % {$_.replace("_","\_")} > release_notes.txt
...
if ($git_major_version -eq $major_version -and $git_minor_version -eq $minor_version) {
    $commit_count = (git rev-list "$latest_tag..HEAD" --grep="^\[#" --invert-grep --count)
